...
function KpiCard({ label, value, sub, hidden }) {
  return (
    <div className="bg-white rounded-2xl border border-slate-200 p-5">
      <p className="text-xs font-semibold uppercase tracking-wider text-slate-400 mb-1">{label}</p>
      <p className="text-3xl font-bold text-slate-800">
        {hidden ? <span className="tracking-widest text-slate-300">••••</span> : value}
      </p>
...
function SourceChart({ dist, total, hidden }) {
  return (
    <div className="bg-white rounded-2xl border border-slate-200 p-5">
          ...
          return (
            <div key={source}>
              <div className="flex justify-between text-xs text-slate-600 mb-0.5">
                ...
                <span>{hidden ? '•• (••%)' : `${count} (${pct.toFixed(0)}%)`}</span>
              </div>
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        ...
        <div className="flex items-center gap-2 flex-wrap">
          ...
          <button
            ...
            title={hidden ? 'Mostrar números' : 'Ocultar números'}
            className={`p-2 rounded-xl border transition ${hidden ? 'bg-amber-50 border-amber-300 text-amber-600' : 'bg-white border-slate-200 text-slate-500 hover:bg-slate-50'}`}
          >
            {hidden ? (
              ...
              {totalPages > 1 && (
                <div className="flex items-center justify-between px-4 py-3 border-t border-slate-100">
                  <p className="text-xs text-slate-400">
                    {hidden ? '•• – •• de •• leads' : `${((page - 1) * PAGE_SIZE) + 1}–${Math.min(page * PAGE_SIZE, leads.length)} de ${leads.length} leads`}
                  </p>
